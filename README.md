...
Люди могут привыкнуть к любым стилям форматирования, но было бы лучше если бы этого не приходилось делать и форматированию уделялось как можно меньше времени. Идеально, чтобы все использовали одинаковый стиль.
Вопрос в том, как это сделать без многостраничных стандартов.

В Go мы используем нетипичный подход и передаем заботу о форматировании машине. 
Программа `gofmt` (также доступна, как `go fmt`) производит действия на уровне пакета, а не на уровне файлов.
Она читает исходный код на Go и преобразует его в код со стандарным стилем отступов и вертикальным выравниванием, сохраняет, и при необходимости, переформатирует комментарии.
Чтобы узнать, правильно ли структурирован ваш код, запустите `gofmt`; 
Если форматирование сделано не по стандарту, `gofmt` исправит вашу программу или сообщит об ошибке. Всегда используйте `gofmt` для форматирования исходного кода.
...
#### Круглые скобки
Go нуждается в меньшем количестве круглых скобок, чем C и Java: структуры ветвления, цикла ( `if` , `for` , `switch`) не имеют круглых скобок в своем синтаксисе. Также, иерархия операторов стала проще и короче. К примеру, выражение
...
Но не будет ошибкой создание геттеров и сеттеров самому, и если это необходимо то делайте так, но идиоматически нет необходимости добавлять ```Get``` в имя геттера. Если у вас есть поле с именем ```owner``` (с маленькой буквы, не экспортируемый), то геттер может называться ```Owner``` (с большой буквы, экспортируемый), а не ```GetOwner```.
...
Мы выполняем кусочки независимо в цикле, по одному CPU на кусочек.
