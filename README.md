...
В Go цикл `for` очень похож, но не такой же как в C. Он унифицирует `for` и `while`, при это отсутствует `do-while` цикл. Существует 3 различных формы и только в одной из них используется точка с запятой.
