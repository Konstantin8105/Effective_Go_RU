...
Ветвление в Go похоже на С, но отличается.
Go не имеет ```do``` или ```while``` циклов, лишь обобщенные ```for``` , ```switch``` которые более гибкие; ```if``` и ```switch``` допускают отсутствие инициализации как в ```for```; ```break``` и ```continue``` могут иметь ярлыки для индентификации чего именно прервать или продолжить; также есть ветвтвление с помощью переключителя типов - ```type switch``` и многоканальный коммуникатор ```select```.

Синтаксис также немного отличается: нет никаких круглых скобок и тело всегда должно иметь разделители - фигурные скобки ```{ }```.

### If

В Go, простое использование условия ```if``` выглядит так:
...
Обязательные фигурные скобки поддерживают написание простых условий ```if``` на несколько строк. Это хороший стиль в любом случае, особенно когда тело содержит управляющий оператор, такие как ```return``` или ```break```.
...
В библиотеках Go, вы увидете - когда условие ```if``` не переходит в следующее условие, то есть в теле используется ```break```, ```continue```, ```goto```, или ```return``` — ненужное ```else``` опускается.
...
Последний пример предедущего раздела  - показывает короткую форму объявления переменных ```:=```. Вызов ```os.Open``` объявляет сразу две переменных ```f``` и ```err```.

```golang
f, err := os.Open(name)
```

Несколько строк до этого, вызов чтения ```f.Stat``` было произведено с объявлением двух переменных ```d``` и ```err```.
...
Это дублирование разрешено: ```err``` в первом случаи объявляется в первую очередь, и переназначаеться во втором.
То есть вызов ```f.Stat``` использует уже существующую переменную ```err``` и просто добавляет новое значение.

В объявлении ```:=``` переменной ```v``` допустимо использовать, даже если она уже объявлена, при условии:

  * если объявление в том же самом области видимости что и существующее объявление переменной ```v``` (если ```v``` уже объявлена за пределами видимости, то объявляя создасться новая переменная §),
 **TODO: перевести**
  * the corresponding value in the initialization is assignable to ```v```, and
  * there is at least one other variable in the declaration that is being declared anew.
**-**

Эта не типичное решение основано из чистого прагматизма - сделать использование переменной ```err``` простым, к примеру при длинных условиях ```if-else```.
...
Имеет 3 различных формы и только в одной из них использует точку с запятой.
...
Если Вы используете цикл по массиву, срезу, строке или мапу(карте, map) или читаете из канала, то для управления можно использовать ```range```.
...
Если Вы будете использовать *range* только для первого элемента (ключ или индекс), то можете пропустить второй элемент:
...
Если Вы будете использовать *range* для второго элемента, то используйте *пустой идендификатор* (**_**) для первого элемента:
...
Использование в цикле по строке - ```range``` делает для Вас намного больше, к примеру разделяет строку по символам Unicode в соответствии с UTF-8. При ошибочном использование кодировки по-байтово и заменяет рунами(*rune*) U+FFFD. (```rune``` в терминологии Go используеться для работы с символами Unicode. Смотрите детальную информацию [Спецификация языка](https://golang.org/ref/spec#Rune_literals)).
...
В связи с тем что отсутствует автоматическое пропускане условий, поэтому условия могут представлены через запятую.
...
В заключении, метод сравнивания байтовых срезов использующий два переключателя ```switch```:
