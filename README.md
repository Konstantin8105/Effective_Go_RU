...
Если Вы хотите знать - как справиться с новой ситуацией размещения кода, то запустите ```gofmt```; если ответ не будет правильным, то <code>gofmt</code> поправить Вашу программу (или файл сообщит об ошибки ```gofmt```), не работайте в обход форматирования программой ```gofmt```.
