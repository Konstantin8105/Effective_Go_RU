...
Управляющие структуры в Go аналогичны тем же струтурам в C, но имеют ряд важных отличий. Во-первых нет циклов `do` и `while`, есть лишь обобщенный `for`. Во-вторых, `switch` более гибкий. В-третьих `if` и `switch` имеют опциональную инициализацию переменных, как и в `for`. В-четвертых, `break` и `continue` опционально принимают метку, к которой небходимо перейти. В-пятых, есть новые операторы, такие как типизированный `switch` и многоканальный `select`. Синтаксис также немного отличается: отсутствуют круглые скобки в условии, и тело структуры всегда должно быть ограничено фигурными скобками.
...
В библиотеках Go, вы найдете подобную запись, если `if` не переходит в следующий блок, т.е. в теле используется `break`, `continue`, `goto` или `return`, а необязательный `else` опускается.
...
Последний пример предедущего раздела демонстрирует использование краткой формы объявления переменных `:=`. Вызов `os.Open` объявляет сразу две переменных `f` и `err`
...
который выглядит как объвления двух переменных `d` и `err`. Хотя `err` присутствует в обоих объявлениях. Это дублирование вполне законно: `err` объявляется в первом случае, и лишь переприсваивается во втором. Это означает, что `f.Stat` использует уже существующую переменную `err`, определенную выше, и просто присваивает ей
новое значение.
...
Хотя, иногда, надо прервать внешний (по отношению к `switch`) цикл, а не сам `switch`, и в Go это может быть достигнуто путем добавления метки перед циклом, и переходом к этой метке в случае вызова `break`. В следующем примере представлены оба случая:
...
Если вы используете соглашение по-умолчание, то для целых чисел можно использовать обобщенный формат ```%v``` (для "значений"); и результат будет одинаков как для  ```Print``` так и для ```Println```.
...
При печати структуры, с аннотацией ```%+v``` производиться печать полей структуры с их именами и для каждого значение с форматом ```%#v``` печатается значение с полным синтаксисом Go.
